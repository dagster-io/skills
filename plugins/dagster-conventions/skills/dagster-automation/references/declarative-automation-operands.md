...
True if the asset partition is part of an in-progress run or backfill that has not yet executed it. Combines `run_in_progress()` and `backfill_in_progress()`.
...
True if the asset partition was materialized or observed since the previous evaluation. For observations, only true if the data version changed.
...
True for time partitions within the latest time window. For unpartitioned or non-time-partitioned assets, always true.
...
True if the asset partition will be requested in the current tick. Used internally for run grouping (see [declarative-automation-advanced.md](declarative-automation-advanced.md#run-grouping)).
...
Operands are composed using operators (see [declarative-automation-operators.md](declarative-automation-operators.md)) to build complex conditions:
