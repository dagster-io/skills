...
Find the errors associated with the run by using the `dg api log get` command for the provided run
ID:
...
If there are no errors when filtering with `jq` `select(.level == "ERROR")` then retrieve the full
logs:
